...
def resolve_auth_paths(base_dir: Path) -> tuple[Path, Path]:
    """
    指定されたベースディレクトリを基に、認証情報のパスを決定する。

    Args:
        base_dir (Path): client_secrets.jsonとtoken.jsonを含むディレクトリ。
    ...
    # ファイルパスの決定
    client_secrets_path = resolved_dir / "client_secrets.json"
    token_path = resolved_dir / "token.json"

    # 既存のロジックを維持: client_secrets.jsonがない場合はエラー
